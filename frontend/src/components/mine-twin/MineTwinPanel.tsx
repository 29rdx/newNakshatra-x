...
import HistoricalForecastGlassVisualizer from './HistoricalForecastGlassVisualizer'
import {
  Cpu,
  Layers,
  Sparkles,
...
export default function MineTwinPanel({ selectedMine = DEFAULT_MINE }: Props) {
  const [activeSection, setActiveSection] = useState<'forecast' | 'simulator'>('forecast')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-2 rounded-2xl bg-[#060D1E]/90 border border-white/20 backdrop-blur-xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setActiveSection('forecast')}
            className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-mono font-bold transition-all flex items-center gap-2 cursor-pointer ${
              activeSection === 'forecast'
                ? 'bg-gradient-to-r from-[#00FF88] to-[#38BDF8] text-black font-extrabold shadow-[0_0_20px_rgba(0,255,136,0.5)]'
                : 'text-slate-300 hover:text-white hover:bg-white/10'
            }`}
          >
            <TrendingUp size={16} />
            <span>50-Year History & 2040 AI Forecast</span>
          </button>
          ...
          >
            <Box size={16} />
            <span>3D What-If Haulage Simulator</span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-slate-300 text-xs font-mono">
          ...
          <span>Context: <strong className="text-white">{selectedMine.name} Mine</strong> ({selectedMine.code})</span>
        </div>
      </div>

      {/* Render 50-Year History & 2040 Forecast Glass Feature */}
      {activeSection === 'forecast' && <HistoricalForecastGlassVisualizer />}

      {/* Render 3D Discrete Event Simulator Panel */}
      {activeSection === 'simulator' && (
